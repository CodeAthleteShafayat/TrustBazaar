-- ============================================================
-- CLEAN_MIGRATIONS.sql
-- Re-run from an empty database. Idempotent (safe to re-run).
-- Supersedes ALL_MIGRATIONS.sql + RECOVERY_MIGRATIONS.sql.
-- ============================================================

-- ===== Extensions FIRST (before any table or index that needs them) =====
...
create table if not exists public.listings (
  ...
  rent_per_day numeric(12,2) check (rent_per_day >= 0),
  deposit_required boolean not null default false,
  deposit_rate numeric(4,2) check (deposit_rate between 0 and 1),
  photo_urls text[] not null default '{}',
  status text not null default 'active' check (status in ('active','sold','rented','archived')),
  location text,
...
-- ===== Backfill: demo accounts (auth.users already exists from earlier signups) =====
